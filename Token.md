...
* Указываемый платеж был успешно оплачен, через Пластиковые карты с помощью шлюза PayU - СТЦ.
* Создать токен можно только для платежей, которые были оплачены в течении 18000 сек.

#### Ответ системы при создании токена
Ответ на запрос создания токена приходит в формате JSON.<br />
<b>Answer Fields</b> - поля передаваемые в ответе запроса, описаны в таблице ниже:<br />
...
````
curl  http://lk.payin-payout.net/api/rpay/token --data 'agentId=719&orderId=999&agentTime=16:41:13 13.09.2016&sign=a9fbcfad642cecb116741454fb13cdb0'
...
````
{"status":true,"result":"5sypd0bdjw9fz1gwprxrxy5v7teu70mb"}
````
